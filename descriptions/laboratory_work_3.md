...
1. **Структура и содержимое Jupyter Notebook**:
    ...
    - Загрузить и обучить 4 нейронные сети (можно использовать `torchvision`). Для каждой модели провести 2 эксперимента: первый — обучить всю модель со случайными весами, второй — реализовать перенос знаний, используя веса, обученные на ImageNet (выбрать любую модель, подходящую для переноса знаний). В этом случае можно обучать только классификатор; всю модель; используя предобученные веса, несколько последних слоев и т.д. Любой разумный вариант с использованием предобученных весов.
...
- Количество баллов за лабораторную работу рассчитывается по формуле:  
  $$\text{баллы} = 10 \times \text{знания} \times \text{лучшая точность}$$, 

  $$\text{знания} \in [0, 1]$$ — оценка уровня понимания материала, 
  $$\text{лучшая точность} \in [0, 1]$$ — лучшая точности решения задачи среди всех экспериментов. Например, точность классификации 90% — это 0.9.
